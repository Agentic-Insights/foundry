...
Complex extraction scenarios including hierarchical structures, dynamic types, tool calling, and streaming.
...
function ExtractInvoice(doc: image) -> Invoice {
  ...
  prompt #"
    ...
    Calculate:
    - Line total = quantity x unit_price
    - Subtotal = sum of line items
    - Tax = subtotal x tax_rate
    - Total = subtotal + tax
...
**CRITICAL: Image parameters MUST be included in prompt body**
...
client<llm> ResilientClient {
  provider fallback
  options {
    strategy [PrimaryGPT, BackupClaude]
  }
}
```
...
**Purpose**: Complex extraction patterns for code generation
**Related**: `pattern_library.md` (pattern storage), `validation_patterns.md` (@assert/@check)
